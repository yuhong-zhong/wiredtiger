...
int
__wt_ebpf_bt_read(WT_SESSION_IMPL *session, WT_ITEM *buf, const uint8_t *addr, size_t addr_size, uint8_t *ebpf_data);

static int
__ebpf_page_read(WT_SESSION_IMPL *session, WT_REF *ref, uint32_t flags, uint8_t *ebpf_data)
{
    WT_ADDR_COPY addr;
    WT_DECL_RET;
    WT_ITEM tmp;
    ...
    WT_ERR(__wt_ebpf_bt_read(session, &tmp, addr.addr, addr.size, ebpf_data));
    if (timer) {
...
{
    return __ebpf_page_read(session, ref, flags, NULL);
}
...
read:
            ...
            WT_RET(__ebpf_page_read(session, ref, flags, ebpf_data));
...
{
    return __wt_ebpf_page_in_func(session, ref, flags, NULL
    #ifdef HAVE_DIAGNOSTIC
      , func, line
    #endif
      );
