...
__wt_connection_open(WT_CONNECTION_IMPL *conn, const char *cfg[])
{
    ...
    atomic_store(&on_disk_tree_access, 0);
    atomic_store(&any_on_disk_tree_access, 0);
...
__wt_connection_close(WT_CONNECTION_IMPL *conn)
{
    ...
    printf("on_disk_tree_access: %ld\n", atomic_load(&on_disk_tree_access));
    printf("any_on_disk_tree_access: %ld\n", atomic_load(&any_on_disk_tree_access));
